...
- **F-IO-06 (INFO)** : `RedisCacheService.delByPrefix` (`redis-cache.service.ts:78-96`) uses `SCAN`+`DEL` paginated loop. SAFE in single-instance mode. In **Cluster mode** (`createRedisClusterClient`), `SCAN` runs per-node and `DEL(...keys)` requires same-slot keys — current `LlmCacheServiceImpl.invalidateMuseum` uses prefixes like `llm:v3:museum-mode:42:` (versioned via `KEY_VERSION`) which scatters across slots → would throw `CROSSSLOT` in cluster. Project does not run Cluster in V1, but flag here so the next refactor toward cluster picks up either hash-tags or a per-slot scan. LOW (latent).
