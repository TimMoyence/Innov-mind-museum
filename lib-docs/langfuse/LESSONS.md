...
- **LF-V3-05 (no `mask` hook)** → **RECLASSED P0 → CLOSED 2026-05-21** by run /team `2026-05-21-p0-c1-pii-egress`. The 2026-05-18/20 LOW-deferred classification raisonnait spans hand-codés (metadata-only, PII-safe) ; LF-V3-02 closure (2026-05-18 CallbackHandler wiring) avait ré-ouvert le vecteur PII via les input/output auto-capturés par `langfuse-langchain.CallbackHandler.handleLLMEnd` (input.messages[*].content, output.text) sans que l'audit antérieur ne re-côte. Fix landed : `mask: stripFreeText` wired in ctor (`museum-backend/src/shared/observability/langfuse.client.ts:55`, helper `museum-backend/src/shared/observability/strip-free-text.ts`). Le mask remplace les champs free-text par `'[STRIPPED]'` tout en préservant `metadata`, `model`, `usage`, `usageDetails` (cost UI intacte). Test seed-PII verrouille l'invariant (`museum-backend/tests/integration/observability/langfuse-pii-seed.test.ts`). Spec/design : `team-state/2026-05-21-p0-c1-pii-egress/{spec,design,tasks}.md`.
