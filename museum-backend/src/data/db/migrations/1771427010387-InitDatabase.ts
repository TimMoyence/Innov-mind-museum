...
export class InitDatabase1771427010387 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`CREATE TABLE "image_insight_conversations" ("id" uuid NOT NULL DEFAULT uuid_generate_v4(), "imageUrl" character varying, "createdAt" TIMESTAMP NOT NULL DEFAULT now(), "userId" integer, CONSTRAINT "PK_05c8a3ab549dbd9cbb3a90c49c6" PRIMARY KEY ("id"))`);
        await queryRunner.query(`CREATE TABLE "chat_sessions" ("id" uuid NOT NULL DEFAULT uuid_generate_v4(), "locale" character varying(32), "museumMode" boolean NOT NULL DEFAULT false, "createdAt" TIMESTAMP NOT NULL DEFAULT now(), "updatedAt" TIMESTAMP NOT NULL DEFAULT now(), "userId" integer, CONSTRAINT "PK_efc151a4aafa9a28b73dedc485f" PRIMARY KEY ("id"))`);
        await queryRunner.query(`CREATE TABLE "chat_messages" ("id" uuid NOT NULL DEFAULT uuid_generate_v4(), "role" character varying(20) NOT NULL, "text" text, "imageRef" text, "metadata" jsonb, "createdAt" TIMESTAMP NOT NULL DEFAULT now(), "sessionId" uuid, CONSTRAINT "PK_40c55ee0e571e268b0d3cd37d10" PRIMARY KEY ("id"))`);
        await queryRunner.query(`CREATE TABLE "artwork_matches" ("id" uuid NOT NULL DEFAULT uuid_generate_v4(), "artworkId" character varying(128), "title" character varying(256), "artist" character varying(256), "confidence" double precision NOT NULL DEFAULT '0', "source" character varying(512), "createdAt" TIMESTAMP NOT NULL DEFAULT now(), "messageId" uuid, CONSTRAINT "PK_0573b35aae5d2896f3999b24512" PRIMARY KEY ("id"))`);
        await queryRunner.query(`ALTER TABLE "image_insight_messages" ADD CONSTRAINT "FK_30f03922716e25cfaefb72a9e5f" FOREIGN KEY ("conversationId") REFERENCES "image_insight_conversations"("id") ON DELETE CASCADE ON UPDATE NO ACTION`);
        await queryRunner.query(`ALTER TABLE "image_insight_conversations" ADD CONSTRAINT "FK_be40cc010dd6fd7c67bb8bebd91" FOREIGN KEY ("userId") REFERENCES "users"("id") ON DELETE NO ACTION ON UPDATE NO ACTION`);
    ...
    public async down(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`ALTER TABLE "image_insight_conversations" DROP CONSTRAINT "FK_be40cc010dd6fd7c67bb8bebd91"`);
        await queryRunner.query(`ALTER TABLE "image_insight_messages" DROP CONSTRAINT "FK_30f03922716e25cfaefb72a9e5f"`);
