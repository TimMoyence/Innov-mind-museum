...
/** Input payload for the internal B2B-lead notification template (R4 §3.4). */
...
export function buildB2bLeadEmail(input: B2bLeadEmailInput): string {
  ...
  const safeMuseum = escapeHtml(input.museum);
  ...
  const bodyHtml = `
    <p style="margin:0 0 20px 0;color:#334155;">A new B2B lead just landed via the museum pitch page.</p>
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="background-color:#F8FAFC;border:1px solid #E2E8F0;border-radius:10px;padding:16px 20px;margin:0 0 24px 0;">
      ${renderFieldRow('Museum', safeMuseum)}
      ${renderFieldRow('Role', safeRole)}
  ...
  return renderEmailLayout({
    heading: 'New B2B lead',
    bodyHtml,
    preheader: `${safeMuseum} via the public B2B pitch form`,
    locale: 'en',
    footerNote: 'Internal notification — do not forward outside the team.',
  });
}
