...
## [Unreleased] — 2026-05-24 — PR-16 `confidenceUpsert<T>` shared helper + sweep 2 knowledge-extraction TypeORM repos (DRY-refactor, byte-equivalent merge)

Run `2026-05-23-pr-16-confidenceUpsert` — seizième et dernier incremental refactor de l'audit `2026-05-23-audit-kiss-dry-backend` (finding B10-#1 : algorithme confidence-based upsert dupliqué ~87 LOC entre `typeorm-artwork-knowledge.repo.ts` et `typeorm-museum-enrichment.repo.ts`). Pipeline UFR-022 fresh-context : Spec → Plan → Red (tests qui FAIL) → Green → Verify. **Note process** : la phase Green a subi 3 coupures réseau consécutives (socket errors) sur les agents spawnés ; complétée directement par le Tech Lead après récupération de l'état disque (red tests + manifest intacts), en respectant le frozen-test contract. Un **BLOCK-TEST-WRONG** légitime a été traité pendant la finalisation : les tests RED reposaient sur `makeArtworkKnowledge({artist: null})` / `makeMuseumEnrichment({summary: null})` pour produire des champs `null`, mais la factory `extraction.fixtures.ts` utilise `overrides?.x ?? default` (l'opérateur `??` discarde les overrides `null`) et n'assigne jamais le champ P3 `summary` ; le setup des 2 cas concernés a été corrigé (champs null posés explicitement post-factory), assertions intactes, manifest re-hashé. Comportement merge préservé **byte-equivalent** : helper pur/sync `@shared/db/confidence-upsert` (mute `existing` in-place + retourne la même référence ; caller garde find + save → pas de gotcha TypeORM `.set({field: undefined})`). Net : +1 helper (~75 LOC), 2 repos allégés (~87 LOC inline → 2 appels délégués), le `eslint-disable @typescript-eslint/no-unnecessary-type-parameters` du museum repo (closure `fillIfNull<K>`) ÉLIMINÉ (variance résolue proprement via cast borné `Record<keyof T, unknown>`, pas de disable, pas de `as any`). Zéro migration DB, zéro lib bump, zéro OpenAPI delta, zéro FE follow-up.

### Added

- **`museum-backend/src/shared/db/confidence-upsert.ts`** (NEW, ~75 LOC, pur/sync, colocalisé avec `jsonb-validator.ts`/`optimistic-lock-retry.ts`) — `confidenceUpsert<T extends {confidence; needsReview}>(existing, data, opts: {sourceUrl, nullableFields, preserveFields, sourceUrlsField?}): T`. Algorithme byte-equivalent aux 2 repos : (1) dedup-append `sourceUrl` (immutable spread), (2) `data.confidence > existing.confidence` strict → `Object.assign(existing, data, <preserveFields depuis existing>)`, (3) sinon backfill `nullableFields` où `existing[k] === null && data[k] !== null`, (4) toujours `existing.needsReview = data.needsReview`. `nullableFields`/`preserveFields` EXPLICITES (jamais dérivés de l'entité) → les colonnes nullable hors-backfill (museum `summary`/`wikidataQid`/`phone`/`imageUrl`, artwork `roomId`) restent intactes. Variance per-key résolue via `(existing as Record<keyof T, unknown>)[k] = data[k]` (cast structurel borné, NFR-2 sans `eslint-disable`).
- **`museum-backend/tests/unit/shared/db/confidence-upsert.test.ts`** (NEW, FROZEN, sha256 `c9de4ee9…`) — 9 scénarios : overwrite-preserve (artwork id/createdAt/sourceUrls + museum museumId), equal-confidence → backfill (strict `>`), lower-confidence backfill nulls only, sourceUrls dedup-append immutable, needsReview always (2 branches), P3 hybrid guard (summary hors nullableFields reste null).
- **`museum-backend/tests/unit/architecture/pr16-confidenceUpsert-sentinel.test.ts`** (NEW, FROZEN, sha256 `394a0835…`) — sentinel fs-based : les 2 repos ne contiennent plus `data.confidence > existing.confidence` inline + importent `confidenceUpsert`. Verrouille la non-réintroduction (UFR-016). Grep global : 0 occurrence du pattern hors helper.

### Changed (PR-16)

- **`typeorm-artwork-knowledge.repo.ts`** + **`typeorm-museum-enrichment.repo.ts`** — `upsertFromClassification` délègue à `confidenceUpsert` (find + save gardés caller-side, lookup d'identité divergent préservé). Museum passe `preserveFields: ['id','museumId','sourceUrls','createdAt']` (museumId en plus). Le `eslint-disable no-unnecessary-type-parameters` (Approved-by tim@2026-05-05) du museum repo supprimé.
...
- **`museum-backend/src/shared/http/fetch-with-timeout.ts`** (NEW, 48 LOC incl. tsdoc, domain-agnostic) — une fonction exportée `export async function fetchWithTimeout(url: string | URL, init: RequestInit, timeoutMs: number, fetchImpl: typeof fetch = fetch): Promise<Response>`. Arme un `setTimeout(() => controller.abort(), timeoutMs)` interne ; compose le signal appelant via `init.signal ? AbortSignal.any([init.signal, controller.signal]) : controller.signal` (path par défaut sans signal appelant = `controller.signal` brut, byte-identical à l'inline d'origine) ; appelle `fetchImpl(url, { ...init, signal })` (shallow spread — NE MUTE PAS `init`) ; `clearTimeout(timer)` dans `finally` (success ET error ET abort). N'inspecte PAS `response.ok` (responsabilité de l'appelant). `fetchImpl` injectable conserve le hook DI `fetchFn` des adapters. Aucun import (globals Node uniquement : `AbortController`/`AbortSignal.any`/`setTimeout`/`clearTimeout`/`fetch`), pas de `any`, pas de `eslint-disable`. tsdoc documente les 3 sites de divergence + les chemins source-of-truth spec/design.
- **`museum-backend/tests/unit/shared/http/fetch-with-timeout.test.ts`** (NEW, sha256 `cb1bbf5a28cea879446c285fde1d54a790af747b9c1c7f6f67c8332fe2197425`, FROZEN au RED) — 5 cas behaviour : **T5.1** resolve + clear timer on success, **T5.2** abort `AbortError` au deadline + clear timer, **T5.3** clear timer quand fetch reject (network), **T5.4** composition `AbortSignal.any` (abort du signal appelant abort le fetch), **T5.5** non-mutation de `init` (deep-equal pré-call, pas de propriété `signal` ajoutée). Fake timers + mock `fetchFn`.
- **`museum-backend/tests/unit/architecture/pr14-fetchWithTimeout-sentinel.test.ts`** (NEW, sha256 `74ceeda0d3a607352d8d6ac8dfd4091b538f7db565282219d95783de6b967ea9`, FROZEN au RED) — 7 cas sentinel filesystem (`readFileSync`) : les 2 fichiers swept (llama-prompt-guard, presidio) NE DOIVENT PLUS contenir `new AbortController` / `setTimeout(... controller.abort)` et DOIVENT contenir `fetchWithTimeout` ; les 3 fichiers de divergence (replicate, siglip-onnx, llm-guard) DOIVENT porter le littéral `does NOT use \`fetchWithTimeout\``. Empêche un futur "sweep more" de plier les 3 divergences sans relire la rationale.

### Changed

**Sweep — 2 adapters guardrails passent du pattern timer inline à une délégation `fetchWithTimeout`** :

| Adapter | Fichier | Méthode | Net LOC | Note |
|---|---|---|---|---|
| Llama Prompt Guard | `museum-backend/src/modules/chat/adapters/secondary/guardrails/llama-prompt-guard.adapter.ts` | `callClassify()` | **−9** | `import { fetchWithTimeout } from '@shared/http/fetch-with-timeout'` ajouté ; bloc `try/finally clearTimeout` retiré, `fetchWithTimeout(url, init, this.timeoutMs, this.fetchFn)` à la place. Le `!response.ok → throw non_ok_${status}`, le json parse et la validation `malformed_classify_response` IDENTIQUES (dé-indentés hors du `try` retiré). |
| Microsoft Presidio | `museum-backend/src/modules/chat/adapters/secondary/guardrails/presidio.adapter.ts` | `requestWithTimeout()` | **−5** | Même import + même délégation `fetchWithTimeout(url, init, this.timeoutMs, this.fetchFn)` ; `!response.ok → throw` + `return response` inchangés. |

**Divergence-comment-only — 3 adapters NON swept (aucune ligne exécutable modifiée)** :

| Adapter | Fichier | Scope | Raison de la divergence (verbatim commentaire) |
|---|---|---|---|
| Replicate | `museum-backend/src/modules/chat/adapters/secondary/embeddings/replicate.adapter.ts` | `encode()` | "single budget shared across multi-fetch flow (createPrediction + awaitTerminal polling). The helper arms a per-call timer; here the same `controller` must span N fetches." |
| SigLIP ONNX | `museum-backend/src/modules/chat/adapters/secondary/embeddings/siglip-onnx.adapter.ts` | `encode()` | "signal feeds `runWithTimeout` wrapping `session.run` (onnxruntime API), not a `fetch` call. The helper is fetch-specific (Response return type); onnxruntime exposes no fetch surface." |
| LLM Guard | `museum-backend/src/modules/chat/adapters/secondary/guardrails/llm-guard.adapter.ts` | `scanOverHttp()` | "chaos injection aborts the caller-held `controller` BEFORE fetch is invoked. The helper owns its internal controller and exposes no pre-fetch abort handle, so chaos drills must keep the inline pattern to mirror real outage timing." |

- **Behaviour-preservation des 2 sites swept** : valeur de retour, exceptions levées (`non_ok_${status}`, `malformed_classify_response`), compteurs metrics (`_metricsRequests`/`_metricsBlocks`/`_metricsErrors`), log lines et la forme `init.signal` vue par le mock `fetchFn` sont byte-identical pre/post. Le path par défaut (aucun signal appelant — les 2 sites swept) passe `controller.signal` brut → forme inchangée pour les mocks. 95/95 tests adapter (swept + diverged) verts, count inchangé vs main.

### Security

- **Posture fail-CLOSED (ADR-047) préservée byte-identical** — le helper lève `AbortError` sur timeout / re-lève l'erreur sous-jacente sur reject ; les catch blocks des 2 adapters swept mappent toujours vers `failClosed('service_unavailable')`. `classifyError()` mappe toujours `'abort'` → `timeout`, `malformed_` → `malformed`, sinon `network` — le reject induit par abort matche toujours son heuristique `.toLowerCase().includes('abort')`. Aucun amendement ADR-047 nécessaire.
- **Non-mutation de `init` (T5.5)** — shallow spread `{ ...init, signal }` : l'objet `init` de l'appelant n'est jamais muté, pas de fuite de `signal` dans un `init` partagé. Empêche une fuite de listener / une réutilisation accidentelle de signal entre appels.
- **Composition dual-abort `AbortSignal.any` (T5.4)** — préserve à la fois le signal appelant et le timeout interne ; le timer est toujours `clearTimeout` dans `finally` (T5.1/T5.2/T5.3) → aucune fuite de listener/timer.
- **`fetchImpl` DI type-safe** — `fetchImpl: typeof fetch = fetch` ; les adapters passent toujours `this.fetchFn` constructeur-injecté ; aucun guard runtime requis.
- **Aucune nouvelle surface secret/PII** — le helper ne manipule que url/init/timeout/fetchImpl ; le logging reste dans les adapters, intouché.
- **Dépendance runtime** — `AbortSignal.any` requiert Node ≥ 20.3 ; le projet tourne Node 22 (vérifié runtime v22.14.0, `typeof AbortSignal.any === 'function'`). Sûr.

### Migration notes

- Aucune migration DB applicative, aucun rename Redis keyspace, aucun delta OpenAPI, aucun delta schema, aucun FE follow-up. Wire format / comportement HTTP des 2 sites swept préservé byte-identical au niveau réseau.
- **Follow-up (hors scope PR-14) tracké** : 2 duplicats `fetchWithTimeout` module-local supplémentaires existent HORS du scope chat, dans `museum-backend/src/modules/museum/adapters/secondary/external/wikipedia.client.ts:21` (signature `(url, timeoutMs)`) et `wikidata-museum.client.ts:67` (signature `(url, init: FetchWithTimeoutInit)` avec `init.timeoutMs`). Signatures locales DIVERGENTES et le sentinel PR-14 ne verrouille que les 5 fichiers chat → un "sweep more" ne les attraperait pas. Candidat pour un futur PR DRY full-repo si désiré, PAS celui-ci.
