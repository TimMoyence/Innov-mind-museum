...
describe('daily-art', () => {
  describe('selectArtworkForDate', () => {
    it('returns a valid artwork shape with all required fields', () => {
      const artwork = selectArtworkForDate(new Date('2025-06-15'), 'en');
    ...
    it('returns the same artwork for the same day', () => {
      ...
      const first = selectArtworkForDate(date, 'en');
      const second = selectArtworkForDate(new Date('2025-03-20'), 'en');

      expect(first).toEqual(second);
    });

    it('returns a different artwork on a different day', () => {
      const day1 = selectArtworkForDate(new Date('2025-01-01'), 'en');
      const day2 = selectArtworkForDate(new Date('2025-01-02'), 'en');
    ...
    it('cycles through the list deterministically based on day of year', () => {
      ...
      const jan1 = selectArtworkForDate(new Date('2025-01-01'), 'en');
      const jan31 = selectArtworkForDate(new Date('2025-01-31'), 'en');
