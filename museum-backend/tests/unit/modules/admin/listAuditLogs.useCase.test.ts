...
const mockRepo = (overrides: Partial<IAdminRepository> = {}): IAdminRepository =>
  ({
    listAuditLogs: jest.fn().mockResolvedValue({ items: [], total: 0, page: 1, limit: 20, totalPages: 0 }),
    ...overrides,
...
describe('ListAuditLogsUseCase', () => {
  ...
  it('throws for non-integer limit', async () => {
    const useCase = new ListAuditLogsUseCase(mockRepo());
    await expect(useCase.execute({ pagination: { page: 1, limit: 1.5 } })).rejects.toThrow('limit must be between 1 and 100');
  });
