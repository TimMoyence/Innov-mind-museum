...
Protocole de construction des mandats agents pour les **9 agents V13** : `architect`, `editor`, `verifier`, `security`, `reviewer`, `documenter`, `learning-curator`, `doc-fetcher`, `doc-curator`.
...
### CONTEXTE
- Phase: {spec|plan|doc-fetch|doc-curate|red|green|verify|security|review|documenter} (pipeline 9-phase unique, mode UFR-022)
...
### ROLE & MODEL
- Role: {architect|editor|verifier|security|reviewer|documenter|learning-curator|doc-fetcher|doc-curator}
- Model: opus-4.7 (architect, reviewer, learning-curator) | opus-4.6 (editor, verifier, security, documenter, doc-fetcher, doc-curator) — UFR-010
...
- Cache stale (>14j OU version drift vs package.json OU manquant) → déclenche doc-fetcher + doc-curator (deux fresh agents). WebSearch fail (offline/404/rate-limit) → WARN + use stale + tag rapport (pas de BLOCK).
- Sortie JSON DOIT inclure `libDocsConsulted[]` couvrant les imports. Le verifier hook `pre-phase-doc-reference-check.sh` BLOCK si la couverture est incomplète.
...
|---|---|---|---|
| **architect** | opus-4.7 | `team-state/<RUN_ID>/{spec,design,tasks}.md`, `team-state/<RUN_ID>/handoffs/*.json`. Spawné 2× : phase=spec → `spec.md` only ; phase=plan → `design.md` + `tasks.md` only. | Source code, deploy, git commit |
| **editor** | opus-4.6 | Source code (museum-{backend,frontend,web}/), CI workflows (.github/workflows/), migrations via CLI, tests, `team-state/<RUN_ID>/handoffs/*.json`. Spawné 2× : phase=red → tests FAIL + `red-test-manifest.json` ; phase=green → code applicatif (frozen-test, ne touche AUCUN test du manifest). | git commit, deploy/SSH/EAS submit, generated/openapi.ts manual edit, modifier un test frozen en phase green |
| **verifier** | opus-4.6 | Read-only on code. `state.json.gates[]` via deterministic hooks only. Vérifie `libDocsConsulted[]` (REGLE 15). | Edit/Write source, git commit |
| **security** | opus-4.6 | Read-only. Reports to STORY.md via documenter handoff. Toujours présent (mode unique). | Edit/Write code, git commit |
| **reviewer** | opus-4.7 (fresh-context) | Read-only. STORY.md `review` section append. Rejection loop ILLIMITÉ (REGLE 14). | Edit/Write code, git commit, sycophancy (UFR-013) |
| **documenter** | opus-4.6 | `docs/`, `README*.md`, `CHANGELOG.md`, `STORY.md`, ADR (`docs/adr/`). Toujours présent (mode unique). | Source code, team-knowledge/, team-reports/ |
| **learning-curator** | opus-4.7 | `team-knowledge/amendments/pending/*.md` only. Read-only sur les règles de production. User-gated (no auto-apply). Invoqué hors-pipeline via `/team learning:review`. | Edit production rules/prompts/hooks, git commit |
| **doc-fetcher** | opus-4.6 | `lib-docs/<lib>/` (snapshots bruts only). Fetch + dump, pas de curation. Parallélisable par lib (read-only sur source). | Edit source/PATTERNS.md, git commit |
| **doc-curator** | opus-4.6 | `lib-docs/<lib>/PATTERNS.md` only. Read-only sur snapshot + LESSONS.md. | Edit source/snapshot/LESSONS.md, git commit |
...
### Phase spec — architect #1 (Opus 4.7), fresh-context
...
### Phase plan — architect #2 (Opus 4.7), fresh-context, zéro mémoire de spec
...
### Phase red — editor #1 (Opus 4.6), fresh-context
...
### Phase green — editor #2 (Opus 4.6), fresh-context, zéro mémoire de red
...
7. Append STORY.md section `implement` + handoff brief vers verifier.
...
| 2.5 | doc-freshness | doc-fetcher + doc-curator (si stale/drift/missing) | `lib-docs/<lib>/{snapshot,PATTERNS.md}` |
| 3 | red | editor #1 | tests FAIL + `red-test-manifest.json` |
| 4 | green | editor #2 | code applicatif (frozen-test) |
| 5 | verify | verifier | `state.json.gates[]` + `libDocsConsulted[]` check |
...
>
> **learning-curator** est hors-pipeline : invoqué via `/team learning:review`, user-gated.

---

## REGLES ABSOLUES

1. Tous les agents en `model: opus-4.7` (architect, reviewer, learning-curator) ou `opus-4.6` (editor, verifier, security, documenter, doc-fetcher, doc-curator) — UFR-010 all-Opus sans exception (le frontmatter utilise l'alias `model: opus`)
2. Les agents ne commitent PAS — seul le Tech Lead `git add/commit/push`
3. Les agents n'écrivent PAS dans `team-knowledge/` ni `team-reports/` (sauf learning-curator → `team-knowledge/amendments/pending/` only, user-gated)
...
11. **Lib-docs obligation (UFR-022, SKILL.md REGLE 15)** : red/green/reviewer consultent `lib-docs/<lib>/PATTERNS.md` + `LESSONS.md` ; `libDocsConsulted[]` vérifié par `pre-phase-doc-reference-check.sh` (verifier). Stale → doc-fetcher + doc-curator.
