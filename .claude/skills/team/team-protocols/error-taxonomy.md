...
2. Determiner la phase de re-spawn (UFR-022 9-phase: spec, plan, red, green, verify, security, review):
   ...
   - E-RUNTIME → re-spawn fresh la phase pointee ; le smoke/E2E est re-execute par le gate verify (hooks) et l'investigation suit `systematic-debugging.md` (instrumenter les frontieres de composants : route → chat.service → guardrail → LLM adapter → cache ; tracer le data-flow jusqu'a la source AVANT tout fix). Au cap correctif (intraPhaseHookLoops ≥ 2) → debug-log.md + question archi (Phase 4.5)
