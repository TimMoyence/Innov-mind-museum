...
- **Wave A — Contenu démo (C1, C2, C3, C4, C4b)** — `DONE 2026-05-21 commit 5a2da5f94, reviewer APPROVED 87.8`
- **Wave C — Cache correctness (I-FIX1, I-FIX2)** — `DONE 2026-05-21 commit 61838dcf2, reviewer APPROVED 90.25`
- **TD-SEC-WAVEA-01 mini-cycle (regex Qid SPARQL guard)** — `DONE 2026-05-21 commit 5f417685f, reviewer APPROVED 93.9 (closes SEC-M1)`
- **Wave B — Multi-tenant (C7, C8, C9)** — `DONE 2026-05-22 commit d128df275, reviewer APPROVED 87.4, BOLA SECURE`
- **Wave C6 — .env release** — `DONE 2026-05-22 commit 048fd904d, pure-config exemption UFR-022 (no review)`
- **Wave C5 — Plausible funnel** — `DONE 2026-05-22 commit 6c985f6fe, reviewer APPROVED 84.4 (override-up, gap = mobile consent UI = TD-C5-MOBILE-CONSENT-01 shipBefore V1)`

### Outstanding TDs (rolled up — full list in respective code-review-*.json)

**HIGH (shipBefore V1 launch 2026-06-07)** :
- `TD-C5-MOBILE-CONSENT-01` — mobile consent banner UI wiring (funnel mobile signals = 0 until shipped)

**MEDIUM (shipBefore V1 launch)** :
- `TD-COR-WAVEB-02` — createTicket route layer missing req.user.museumId plumb (~15 LOC)
- `TD-C5-CONSENT-HEADER-01` — BE X-Musaium-Analytics-Consent header gate (PATTERNS §4 item 4)
- `TD-C5-PROXY-TEST-01` — BE proxy route integration test (bundle w/ consent-header)
- `TD-SEC-WAVEA-02` — bump brace-expansion>=5.0.6 + ws>=8.20.1 (baseline CVEs)

**MEDIUM (post-launch sprint)** :
- `TD-COR-WAVEB-03` — getStats museumId enforce (4 options architectural)

**LOW (post-launch or opportunistic)** :
- `TD-COR-WAVEB-01` — rbac-matrix.test.ts m1→42 (~1 LOC)
- `TD-COR-WAVEB-04` — body z.strictObject migration (~6 LOC)
- `TD-COR-WAVEB-05` — baseline drift sweep (3 hand-authored migrations)
- `TD-DOC-WAVEA-01` — lib-docs typeorm PATTERNS.md + LESSONS.md
- `TD-DOC-WAVEC-01` — CLAUDE.md llm:v1: → llm:v2: stale gotcha
- `TD-COV-WAVEC-01` — 3 narrow tests on chat-message.service.ts threading
- `TD-ARCH-WAVEC-01` — optional 2-arg createCachePurgeRouter signature when needed
- `TD-C5-AUTH-FUNNEL-01` — telemetry funnel spoofing re-eval
- `TD-C5-LINT-FROZEN-01` — 4 frozen-test warnings
- `TD-C5-CSP-NONCE-01` — Plausible script nonce propagation staging smoke
- `TD-C5-PRIVACY-POLICY-01` — Plausible disclosure copy
- `TD-SEC-LANG-01` — `${lang}` Qid validation (fail-safe today, hardcoded 'en')
