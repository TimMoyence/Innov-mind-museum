...
- **CORS allowedHeaders contient déjà `sentry-trace` + `baggage` (museum-backend/src/app.ts:116-117)** — depuis 2026-05-17 W4 W6.9 distributed tracing wire-up. Ne PAS les retirer même si une refacto CORS le suggère : sans eux, le header `sentry-trace` est stripped au preflight et le bridge FE↔BE casse silencieusement (trace ID jamais propagé, investigation cross-system retombe au grep manuel). Le BE middleware `museum-backend/src/shared/observability/trace-propagation.middleware.ts` est aujourd'hui exporté mais PAS encore mounté dans `app.ts` (suivi : applicateur W4 ou suivante PR observability). Référence : `docs/observability/DISTRIBUTED_TRACING.md` §5.
...
This project is indexed by GitNexus as **Innov-mind-museum** (26239 symbols, 43621 relationships, 300 execution flows). Use the GitNexus MCP tools to understand code, assess impact, and navigate safely.
