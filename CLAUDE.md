...
- **LLM response cache = `LlmCacheServiceImpl` only (ADR-036)** — un seul layer use-case-level. Ne PAS réintroduire de décorateur adapter-level (`CachingChatOrchestrator` supprimé). Cache key = `llm:v1:{contextClass}:{museumId|none}:{userId|anon}:{sha256}`. TTL tune = data-driven, ≥7j bake + ADR-036 amendment.
...
This project is indexed by GitNexus as **Innov-mind-museum** (28928 symbols, 46771 relationships, 300 execution flows). Use the GitNexus MCP tools to understand code, assess impact, and navigate safely.
