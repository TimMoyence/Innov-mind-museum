...
- ~~`llm-security-garak.yml`~~ **Supprimé 2026-05-17** — coût réel ~$120/mois (256 prompts × 6 probes × ~18s/call orchestrator full → ~8h wall-clock + ~$30 OpenAI tokens × 4 runs/mois) vs $2/mois estimé. Deferred V2.1 quand LLM Guard sidecar in CI permettra un fast-path target sans full orchestrator. Cf. ADR-049 amendment 2026-05-17.
...
- **Sentry+OTel Node SDK v2 coexistence** — `skipOpenTelemetrySetup: true` + `getDefaultIntegrationsWithoutPerformance()` est REQUIS sinon Sentry tracing collide avec custom OTel NodeSDK. Aussi : `tracePropagationTargets` doit être explicite (`['^https://api.musaium\\.com/']`) sinon trace tree BE↔FE split silencieux. **Précision 2026-05-19 (ADR-045 owner decision)** : la corrélation de traces BE↔FE est assurée par le middleware header-based `museum-backend/src/shared/observability/trace-propagation.middleware.ts`, **PAS** par un SDK bridge `@sentry/opentelemetry` (`SentryPropagator`/`SentryContextManager`/`SentrySpanProcessor`/`SentrySampler` ne sont **pas** câblés et ne doivent pas l'être). `skipOpenTelemetrySetup: true` + `getDefaultIntegrationsWithoutPerformance()` restent corrects et sont l'état-cible définitif. Cf. `docs/HANDOFF-2026-05-19-debt-collision-report.md` §7 décision 1 ; TD-SN-01 marqué STALE-BY-DESIGN.
...
- **CORS allowedHeaders contient déjà `sentry-trace` + `baggage` (museum-backend/src/app.ts:116-117)** — depuis 2026-05-17 W4 W6.9 distributed tracing wire-up. Ne PAS les retirer même si une refacto CORS le suggère : sans eux, le header `sentry-trace` est stripped au preflight et le bridge FE↔BE casse silencieusement (trace ID jamais propagé, investigation cross-system retombe au grep manuel). Le BE middleware `museum-backend/src/shared/observability/trace-propagation.middleware.ts` (`tracePropagationMiddleware`, ex-typo `traceePropagationMiddleware` corrigé 2026-05-19) est monté dans `app.ts:103` depuis le merge W3+W4 (commit `f687b600`). Référence : `docs/observability/DISTRIBUTED_TRACING.md` §5.
- **`infra/grafana/dashboards/*.json` UID est immutable** — chaque dashboard a un `uid:` (ex: `chat-stages-latency` cluster B). Si tu renames le fichier, GARDE l'`uid` historique sinon les liens permanents Grafana cassent + les annotations / alerts qui pointent vers `/d/<uid>` retournent 404. Pareil pour les panels `id:` numérotés dans le tableau `panels[]` — un consumer externe peut deep-link `&viewPanel=<id>`. Référence : `infra/grafana/dashboards/chat-stages-latency.json` schemaVersion 39 (Grafana ≥10).
- **PGP key `museum-web/public/.well-known/pgp-key.txt` placeholder** — depuis 2026-05-17 W4 C8.5, le fichier contient le token `PGP_KEY_PLACEHOLDER_DO_NOT_SHIP`. Le deploy pipeline DOIT gate sur l'absence de ce token avant de push en prod, sinon la PGP key publiée est vide et tous les chercheurs sec essayant d'envoyer un report encrypté reçoivent un block PGP invalide → signal "vendor négligent" qui fait fuir les bons chercheurs. Procédure de génération : `docs/operations/PGP_KEY_GENERATION.md`.
- **`apiPut` n'existe pas dans `museum-web/src/lib/api.ts`** — la stack expose `apiGet` / `apiPost` / `apiPatch` / `apiDelete`. Les pages admin qui doivent PUT (ex: branding 2026-05-17 W4 W2.2) utilisent un wrapper `fetch` local avec injection du CSRF cookie (`csrf_token` → `X-CSRF-Token` double-submit) + `credentials: 'include'`. Si tu vois plus de 2 sites qui ré-implémentent ce wrapper, c'est le signal d'ajouter `apiPut` proprement dans `api.ts` plutôt que de continuer à copier. Réf : `museum-web/src/app/[locale]/admin/museums/[id]/branding/page.tsx` `apiPut`.
...
This project is indexed by GitNexus as **Innov-mind-museum** (29213 symbols, 46715 relationships, 300 execution flows). Use the GitNexus MCP tools to understand code, assess impact, and navigate safely.
