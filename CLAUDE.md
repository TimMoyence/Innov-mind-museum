...
- **LLM response cache = `LlmCacheServiceImpl` only (ADR-036)** — un seul layer use-case-level. Ne PAS réintroduire de décorateur adapter-level (`CachingChatOrchestrator` supprimé). Cache key = `llm:v3:{contextClass}:{museumId|none}:{userId|anon}:{sha256}` (bumpé `v1` → `v2` par commit `d54552beb` 2026-05-19 quand `voiceMode` + `audioDescriptionMode` ont été inclus dans la key ; bumpé `v2` → `v3` 2026-06-12 quand `lowDataMode` a été inclus dans la key — run undefined-network-detection-reliability, US-12.2/INV-21 ; chaque bump invalide l'ancien namespace ; doc CLAUDE.md restait stale jusqu'au P0 lot fix 2026-05-22 / TD-DOC-WAVEC-01). Source de vérité : `museum-backend/src/modules/chat/useCase/llm/llm-cache.service.ts:14` (`KEY_VERSION`). TTL tune = data-driven, ≥7j bake + ADR-036 amendment.
...
- **Une queue BullMQ producteur-seul = fiche vide silencieuse** — `EnrichMuseumUseCase` (cache-miss) `enqueue()` sur `museum-enrichment`, mais le CONSUMER `MuseumEnrichmentWorker` n'était JAMAIS instancié au boot (`grep "new MuseumEnrichmentWorker"` = 0 hits) → les jobs s'empilaient dans Redis, `processMuseumEnrichmentJob → cache.upsert` ne tirait jamais, la fiche restait `pending`/vide (task #16). Fix : `startEnrichmentWorker()` (`src/index.ts`) construit+`start()` le worker dans `bootBackgroundJobs`, gaté sur `env.cache?.enabled` (capability Redis, PAS un feature flag) AVANT tout client ioredis, fail-open en try/catch, `worker.close()` awaité dans `drainAsyncResources`. Factory `buildMuseumEnrichmentWorker(connection)` au composition root museum. Le SCHEDULER périodique (`museum-enrichment-scheduler`, autre queue, `MUSEUM_ENRICHMENT_SCHEDULER_ENABLED`) reste séparé + default-OFF. Garde-fou général : toute queue avec un `add()` doit avoir un `new Worker()` câblé au boot.
...
- **PGP key `museum-web/public/.well-known/pgp-key.txt` = REAL Ed25519 key shipped** — UID `Musaium Security <security@musaium.com>` (plus de placeholder `PGP_KEY_PLACEHOLDER_DO_NOT_SHIP`). Le gate anti-placeholder de déploiement est ACTIVE : `.github/workflows/ci-cd-web.yml:424` (`node scripts/sentinels/wellknown-placeholder-free.mjs`, step « PGP placeholder deploy gate », prod-deploy only) — il bloque tout retour à un `.well-known` placeholder. Ne PAS le retirer. Rotation/régénération : `docs/operations/PGP_KEY_GENERATION.md`.
...
- **Un build e2e est un build RELEASE — donc `__DEV__ === false`, et tout ce qui est gaté dessus est MORT dedans.** Les deux binaires e2e sont des Release (iOS `xcodebuild -configuration Release`, Android `./gradlew assembleRelease` → `--dev false`). Le groupe de routes `(dev)/` était gaté sur `__DEV__` : les flows qui le deeplinkaient **passaient au vert en ne testant rien** (2 routes modales supprimées pour ça en 2026-06, cf. `app/(dev)/_layout.tsx`), et `force-data-mode` a hérité du même défaut — les 4 flows netshape ne forçaient rien et leur assert `low-data-badge` ne **pouvait pas** passer, sur aucune plateforme. Gate correct = seam de build (`EXPO_PUBLIC_E2E_DEV_ROUTES`, `shared/lib/e2eBuildFlags.ts`), inliné par Metro, jamais posé hors des jobs Maestro (sentinel `scripts/sentinels/e2e-flags-not-in-env.mjs`). **Corollaire général : ne jamais gater un seam e2e sur `__DEV__`.**
- **`FREE_TIER_MONTHLY_SESSION_LIMIT` vaut 3 par défaut — et 17 des 18 flows du shard chat ouvrent une conversation.** Chaque shard exécute tous ses flows dans UN job contre UNE base : dès la 4ᵉ conversation le backend répond 402, la `QuotaUpsellModal` recouvre le composer, `chat-input` n'apparaît plus et tous les flows suivants tombent. Plafond dur, pas de la flakiness. La variable doit être levée sur **tout job qui enchaîne >3 flows chat** (iOS, shard chat Android, netshape) — et le seed lit la **même** variable pour caler le compteur du compte paywall, donc les deux processus doivent voir la même valeur sinon `modal-paywall-quota-upsell` perd son 402.
- **Le Keychain iOS survit à `clearState` / une désinstallation.** Il n'est pas dans le container de données de l'app : un refresh token y survit et **ressuscite la session** — l'app boote past `/auth` directement sur Home, et les flows suivants héritent d'un état connecté. Android (`pm clear`) efface tout : les deux plateformes divergent vraiment. Purge côté app **uniquement sur container réellement neuf** (`purgeStaleCredentialsOnFreshInstall`, témoin `musaium.userProfile` — sinon la 1ʳᵉ mise à jour qui embarque le marqueur **déconnecte toute la base installée**). Maestro expose aussi `clearKeychain` sur `launchApp`.
- **`CODE_SIGNING_ALLOWED=NO` produit un binaire SANS entitlements → expo-secure-store est mort.** Pas d'`application-identifier` ⇒ pas de keychain access group ⇒ `setValueWithKeyAsync` échoue (`errSecMissingEntitlement`, -34018) ⇒ le token d'auth ne persiste pas ⇒ **login impossible**. Un build simulateur SANS aucun flag de signature prend l'identité ad-hoc « Sign to Run Locally » : ni cert, ni team, ni secret (marche sur un runner CI nu) et le keychain fonctionne. Ne PAS ajouter `DEVELOPMENT_TEAM` — un build simulateur l'ignore.
- **`hideKeyboard` de Maestro atterrit sur le bouton en dessous du champ (iOS).** Sur l'écran auth il touche `auth-submit` et **déclenche le login en avance** : le step lui-même flake et/ou `auth-submit` est déjà démonté à la commande suivante. L'app ne soumet PAS au return (`LoginForm.tsx` ne câble que `onPress`) — c'est un artefact Maestro. Remède : taper un **texte inerte** (le titre), ce qui dismisse réellement le clavier **parce que** le formulaire vit dans un `ScrollView keyboardShouldPersistTaps="handled"`. **Vérifier cette condition** : dans une modale sans ScrollView (`QuotaUpsellModal` avant fix) le tap est un **no-op** et le test ment.
- **Le runner `macos-26` n'a PAS de runtime iOS 17.5** (uniquement 26.x ; `macos-15` avait 18.x). Tout `simctl create … "iOS17.5"` en dur est une bombe à retardement, et un `|| true` transforme « runtime introuvable » en « No devices are booted » deux steps plus loin. Résoudre le runtime **dynamiquement** (`museum-frontend/scripts/pick-ios-simulator.mjs`) et prendre le device dans la liste `supportedDeviceTypes` **du runtime** (filtrer `productFamily === 'iPhone'` globalement renvoie aussi l'iPod touch → « Incompatible device »).
- **Un appel LLM n'est pas un CRUD : il lui faut son propre timeout.** `httpClient` impose 15 s à tout le monde ; or `POST /chat/sessions/:id/audio` (STT → LLM → TTS en une requête) a été mesuré à **14,1 s contre un backend LOCAL** — 0,9 s de marge. Sur réseau réel, axios abandonne (`ECONNABORTED`) et le message vocal est **perdu silencieusement**. Idem message texte et TTS (qui synthétise la réponse entière avant le 1er octet → les réponses longues sont justement celles qu'il coupe). Utiliser `LLM_REQUEST_TIMEOUT_MS` (`shared/api/httpRequest.ts`) sur tout appel qui attend un modèle.
- **Une auto-instrumentation OTel peut TUER le produit en silence — et `OTEL_ENABLED` est l'unique interrupteur.** `@opentelemetry/instrumentation-openai` (que personne n'a installé : il arrive comme dépendance transitive de `@opentelemetry/auto-instrumentations-node`, **actif par défaut** depuis la 0.66.0, entré par un minor auto-mergé) patche `chat.completions.create()` et fait `.then()` sur l'`APIPromise` du SDK — ce qui **consomme le corps HTTP**. Or le chat ne parle au modèle QUE via `withStructuredOutput()` → `chat.completions.parse()` → `APIPromise._thenUnwrap()` → **2ᵉ lecture du même corps** → `TypeError: Body is unusable`. Mesuré : **0 succès / 42 appels**, chaque réponse = le gabarit vide `createSummaryFallback` — **l'assistant ne répond jamais**, et le judge V2 (fail-open) + le classifieur knowledge-extraction, qui passent par le même chemin structuré, sont **désarmés silencieusement**. Dégâts pendant ~2 mois (INC-2026-07-14). Bug **amont OPEN** (`opentelemetry-js-contrib#3586`) : le patch fautif est byte-identique de 0.15.0 à 0.18.0 → **monter de version ne répare rien**. Fix : `'@opentelemetry/instrumentation-openai': { enabled: false }` dans `src/shared/observability/otel-instrumentation-policy.ts` — **nom COMPLET du paquet obligatoire** (`getNodeAutoInstrumentations` indexe `InstrumentationMap` par nom de paquet ; une clé courte type `'openai'` ne fait qu'un `diag.error` MUET → **no-op silencieux**, l'instrumentation reste armée avec une config qui *a l'air* corrigée ; les variables d'env prennent la forme INVERSE, le suffixe court — d'où le piège). La config programmatique est **priorité 1** : aucun `OTEL_NODE_ENABLED_INSTRUMENTATIONS` ne peut la défaire. Garde-fou : `pnpm sentinel:otel-roster` (Gate 35 pre-push + miroir CI) recalcule le roster depuis le **bundle installé** — pas depuis notre source, pas depuis une doc (`lib-docs/opentelemetry/LESSONS.md` D5 est faux : il prétend que `instrumentation-router` n'est pas dans le bundle, `node_modules` dit le contraire) — et échoue sur tout ajout/retrait/renommage, en **nommant le delta**. **Condition de levée** : le jour où une version publiée cesse de consommer le corps (remède de référence : patcher `_thenUnwrap`, cf. `DataDog/dd-trace-js#5253`), le mode `control` du probe `tests/fixtures/otel/structured-output-probe.ts` passe au vert → **c'est le signal**, pas une panne.
- **`pnpm test` (BE) local ≠ la CI : la CI shard en 4 ET pose `--testTimeout=30000` ; le local ne faisait ni l'un ni l'autre.** Deux faux rouges **local-only** en découlaient, tous deux réparés dans le script `test` (2026-07-14) :
  - **(a) OOM.** 691 suites dans UN process `--runInBand` → pic mesuré **4693 Mo**, au-dessus du plafond Node par défaut (~4 Go) → `FATAL ERROR: Ineffective mark-compacts near heap limit — JavaScript heap out of memory`, `Abort trap: 6`, **exit 134** vers la ~450ᵉ suite, *aucun test rouge* : la suite est simplement tuée. Fix : `NODE_OPTIONS=--max-old-space-size=8192`. Diag : `pnpm exec jest … --logHeapUsage` et lire le **pic**, plutôt que d'accuser la dernière suite ajoutée — le heap monte de façon monotone sur tout le run, la suite qui plante n'est **pas** la coupable.
  - **(b) Flakes de timing.** Le défaut jest est **5000 ms**, alors que `ci-cd-backend.yml` lance `jest … --testTimeout=30000 --shard=N/4`. Sur un run monolithique de ~500 s (GC + 7 400 tests), les suites de routes supertest/rate-limit franchissent les 5 s et tombent en `Exceeded timeout of 5000 ms` — **une suite différente à chaque run** (`admin.route`, `leads.route`, `auth.route` observées), et le timeout **pollue l'état du rate-limiter** → le test suivant reçoit un 400 au lieu d'un 200 (le 2ᵉ échec est du collatéral, pas une 2ᵉ cause). Toutes passent en isolation (`auth.route` seule = 19,4 s). Fix : aligner le script local sur la CI (`--testTimeout=30000`). Aucune assertion n'est relâchée — c'est le timeout de harnais de jest, pas un budget de latence.
  Règle : avant de débugger un rouge de `pnpm test` qui ne reproduit pas en isolation, **compare l'invocation locale à celle de la CI**.
...
- **Un `$` littéral dans une valeur du `.env` lu par Docker Compose = secret TRONQUÉ EN SILENCE** — compose interpole `${VAR}` dans le fichier compose **en utilisant les valeurs du `.env`**. Si un secret contient `…$Y40q6…`, compose y voit une référence de variable, ne la résout pas, émet un `WARN[0000] The "Y40q6" variable is not set. Defaulting to a blank string.` **sur stderr**, sort en **0**, et substitue **du vide**. Le service reçoit un secret **amputé** tandis que le `.env` (et le gestionnaire de mots de passe) portent la version complète. Cas réel prod 2026-07-14 : `GRAFANA_ADMIN_PASSWORD` — personne ne pouvait se connecter à Grafana avec le mot de passe qu'il croyait avoir, et personne ne le savait. **Asymétrie clé** : `env_file:` passe les valeurs **BRUTES** (non interpolées), `environment: ${VAR}` passe la valeur **INTERPOLÉE** → deux consommateurs du même secret peuvent recevoir deux valeurs différentes. **Le gate existait et jetait le signal** : `ci-cd-backend.yml` faisait `docker compose … config > /dev/null` — les warnings partent sur stderr, donc dans le vide (corrigé : le déploiement échoue désormais si `config` émet un `variable is not set`, sans jamais imprimer le fragment de secret dans les logs). **Correctif d'un secret concerné : le régénérer SANS `$`** (le plus sûr) ; `$$` fonctionne pour l'interpolation compose mais casse le chemin `env_file:` (qui livrerait `$$` littéral). Diagnostic sans fuite : comparer les LONGUEURS (`sudo grep '^X=' .env | cut -d= -f2- | wc -c` vs `docker compose exec -T <svc> sh -c 'printf %s "$X" | wc -c'`), et lister les NOMS concernés par `sudo grep -nE '^[A-Z_][A-Z0-9_]*=.*[$]' .env | cut -d= -f1`.
