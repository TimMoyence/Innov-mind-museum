...
Avant ce run, le ctor `new Langfuse({ publicKey, secretKey, baseUrl, flushAt, flushInterval })` (`museum-backend/src/shared/observability/langfuse.client.ts:57-69`) **n'avait pas** la clé `mask` câblée. Conséquence : tout `LANGFUSE_ENABLED=true` (default `false` `config/env.ts:281`, mais activable ops-side) shippait les prompts utilisateur et complétions LLM **non maskés** vers Langfuse. Vecteur PII direct (OWASP LLM07 — PII via tracing ; GDPR Art. 5 §1.c — data minimisation ; CNIL recommandation 2022-06-23 sur observabilité).
