...
>
> **🔁 Réconciliation doc↔code 2026-05-21** (vérifiée grep/read) : 12 items ci-dessous étaient marqués `❌` mais sont en fait **mergés ou réfutés** → re-marqués `✅`. Le texte d'audit d'origine de chaque ligne est conservé tel quel ; le marqueur ✅ + ce bloc font foi.
...
> 5. ✅ **P0-FA5 — images daily-art : 30/30 LIVE (sentinel PASS 2026-05-28)** : le « 14/30 cassées » (audit 2026-05-25) est **STALE** — `scripts/sentinels/artwork-image-liveness.mjs` re-curlé 2026-05-28 → `PASS — all 30 catalog image URLs are live (2xx)`. Sentinel **DÉJÀ câblé CI** : `.github/workflows/artwork-image-liveness.yml` (cron lundi 06:00 UTC + PR touchant le catalogue + `workflow_dispatch`). Seul reste **non-bloquant** : **pas de fallback icône** dans `DailyArtCard.tsx` (hotlinks Wikimedia restent fragiles : 429/renames) — résilience optionnelle. *Texte audit d'origine : « 14/30 cassées, Mona Lisa/Guernica, ~47% jours = fallback icône, re-sourcer + sentinel CI liveness ».*
...
>
> **⚠️ Honnêteté (UFR-013)** : LOT 4 (I-OPS2/3/4/6/7, I-FIX3, I-SEC8) — les claims de complétion sur worktree `p0/stability` sont **non vérifiés sur `dev`** (SHAs non-ancêtres, fichiers introuvables). LOT 4 reste NON DÉMARRÉ sur dev. **[SUPERSEDED 2026-05-28 : FAUX désormais — #300 `34bf280fc` (lot stabilité + I-SEC8, 9 items) EST mergé et ancêtre de HEAD `0e3808a5` (`git merge-base --is-ancestor` ✓). I-OPS7/I-SEC8/I-FIX3 fermés, cf. lignes-items + P1-FA10.]** La note de clôture `LOT-P0-STABILITY-CLOSURE.md` a été retirée 2026-05-26 ; ses actions ops durables (backup média/2ᵉ bucket, exporters PG/Redis, branch-protection sentinel-mirror, DR fresh-DB) sont promues dans [`OPS_DEPLOYMENT.md`](OPS_DEPLOYMENT.md) §28. · Doc-anchors `c4b-sparql-counts.md`/`c2-license-uris.md` (preuves working-dir éphémère purgées) : refs mortes DONE-WHEN des tasks-files p0-feature-gates retirées 2026-05-26 → pointent désormais vers la décision P0.C4b (UFR-024 résolu). · I-SEC8 reclassé CRITIQUE→**LOW** (catalogue public scrapé, ADR-061 cohérent ; consensus B5+C6 vs A2). · I-SEC4/I-SEC6 textes "live" à réécrire (bugs fixés/jamais réels).
...
>
> **Anchors/sentinels (UFR-024)** : `c4b-sparql-counts.md`/`c2-license-uris.md` refs mortes DONE-WHEN retirées 2026-05-26 (preuves working-dir purgées, → décision P0.C4b) · `doc-anchor-check.mjs` jamais livré ; le sentinel doc-anchor réel = `roadmap-claim-resolves.mjs` (scope `docs/ROADMAP*.md`) · LOT 4 stability = commits orphelins non-ancêtres, jamais mergé sur dev **[SUPERSEDED 2026-05-28 : #300 `34bf280fc` est ancêtre de HEAD `0e3808a5` — vérifié `git merge-base --is-ancestor` ; LOT 4 mergé]** (note `LOT-P0-STABILITY-CLOSURE.md` retirée 2026-05-26 ; actions ops → `OPS_DEPLOYMENT.md` §28) · `_rtl-style-audit.ts` ne couvre pas les border-radius (RTL bug `SwipeableConversationCard` non-flaggé) · 2 Maestro stale actifs sur shards CI (`audio-recording-flow.yaml`, `onboarding-flow.yaml`).
...
| ✅🧑‍🔧 **P0.B12** | **✅ CLÉ RÉELLE 2026-05-28** : `museum-web/public/.well-known/pgp-key.txt` = vrai bloc PGP Ed25519 (UID `Musaium Security <security@musaium.com>`), `grep PGP_KEY_PLACEHOLDER_DO_NOT_SHIP` = 0 occurrence. Reste 🧑‍🔧 : **CI gate anti-placeholder à ajouter** au pipeline de déploiement. *Texte d'origine : body = literal `PGP_KEY_PLACEHOLDER_DO_NOT_SHIP` ; gen Ed25519 2y + remplace OU retire `Encryption:` de `security.txt`.* | Agents 09 A+B | 30 min Tim |
...
| ✅ **I-SEC8** | ◇ | **KE knowledge base sans `museum_id` → cross-tenant bleed** : `findById(currentArtworkId)` injectait le `[CURRENT ARTWORK]`/`[LOCAL KNOWLEDGE]` d'un autre musée dans le system prompt. **✅ FERMÉ 2026-05-28 (#300 `34bf280fc`, ancêtre de HEAD)** : `prepare-message.pipeline.ts:359` `findById(currentArtworkId, session.museumId)` + scoping SQL réel `museum_id` (`typeorm-artwork-knowledge.repo.ts:42-44`). Cross-tenant bleed clos. *Texte d'origine : « Reste OUVERT re-vérifié 2026-05-25 D3 ; #294 reclassify mais NE corrige PAS le scoping ».* | `artwork-knowledge.entity.ts:14-67` + `prepare-message.pipeline.ts:359` |
...
| ✅ **I-OPS7** | ◇ | **Indices manquants** : chat-purge cron full seq-scan (`purgedAt`/`updatedAt`) ; `api_keys.user_id` CASCADE FK non-indexé ; `listSessions` sans composite `(userId, updatedAt DESC)`. **✅ FERMÉ 2026-05-28 (#300 `34bf280fc`)** : migration `AddOpsStabilityIndexes` (`1779707124179`, `transaction=false`) crée 3 index `CREATE INDEX CONCURRENTLY IF NOT EXISTS` — `IDX_api_keys_user_id`, `IDX_chat_sessions_userId_updatedAt_id`, partiel `IDX_chat_sessions_purged_at_active WHERE purged_at IS NULL`. | migration `1779707124179-AddOpsStabilityIndexes.ts:38-49` |
