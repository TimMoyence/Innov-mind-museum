...
|---|---|---|---|
| ✅ **P0.A1** | **Email clair-texte** — créé `@shared/pii/extractEmailDomain.ts` + patché les 3 sites in-scope (`forgotPassword.useCase.ts`, `login-handler.helpers.ts`, `auth-password.route.ts`) → `emailDomain` only. Merge `ead8b2ee1`. Follow-up : `auth-email.route.ts:47` `newEmail` brut (LOW, hors scope). | Agent 01 confirmé. Logger sans scrub layer ; audit metadata `LooseRecordSchema`. | 60-90 min |
| ✅ **P0.A2** | **DOB bypass exploitable curl direct** — drop `.optional()` `auth.schemas.ts` + drop fallback `if(!dateOfBirth) return` `register.useCase.ts` (hard-throw 400) + OpenAPI `required[]` + tests réalignés (unit+e2e+perf+contract). Merge `ead8b2ee1`. | Exploit confirmé `curl -X POST /api/auth/register -d '{"email":"minor@test.fr","password":"V1"}'` → 201 + null DOB | 2-3h |
| ✅ **P0.A3** | **Sentry tag URL leak + `event.tags` NON scrubbé** — shipped 2026-05-21 (`/team` run `2026-05-21-p0-c1-pii-egress`, APPROVED 96.25). R1 `SENSITIVE_QUERY_KEYS` étendu 7→11 (`code, state, email, phone` ajoutés, `packages/musaium-shared/src/observability/sentry-scrubber.ts:29-41`). R2 `scrubEvent` traverse `event.tags` (`:220-232`). R3 `captureExceptionWithContext` scrub URL-like tag values via `scrubUrl` upstream de `setTag` (`museum-backend/src/shared/observability/sentry.ts:88-102`). R4 integration test verrouille l'invariant end-to-end. Sentinel `sentry-scrubber-parity.mjs:65` `CANONICAL_HASH` bumpé + `REQUIRED_CANONICAL_EXPORTS` étendu avec `isUrlLikeValue`. | OAuth `/api/auth/google/callback?code=<authcode>&state=<jwt>` leak fermé. | shipped |
| ✅ **P0.A4** | **Langfuse `mask:` SDK option NOT wired + `updateRoot:true` hazard** — shipped 2026-05-21 (même run). R5 `mask: stripFreeText` câblé au ctor (`museum-backend/src/shared/observability/langfuse.client.ts:68`), conforme `langfuse-core@3.38.20 MaskFunction` (signature byte-vérifiée contre types installés au build, cf. `docs/adr/ADR-063-langfuse-mask-central-stripfreetext.md` §References). R6 `stripFreeText` (`strip-free-text.ts:83-147`) scrub free-text LangChain shape (`input.messages[*].content`, `output.text`) + paths manuels (prompt/text/completion/content), préserve `metadata`/`model`/`usage`/`usageDetails`. R7 fail-safe try/catch + `logger.warn('langfuse_mask_failed')`. R8 seed-PII test verrouille l'invariant. R9 `LANGFUSE_ENABLED=false` → mask jamais instancié (0 µs overhead). | `langfuse-langchain.ts:61` `updateRoot:true` confirmé ; grep `mask:` désormais wired. | shipped |
| ✅ **P0.A5** | **Version drift `app.config.ts` literal vs `package.json`** — shipped 2026-05-21 (`/team` run `2026-05-21-p0-c4-infra`, APPROVED 92.75). `museum-frontend/app.config.ts:128` lit désormais `(require('./package.json') as { version: string }).version` (eslint-disable `@typescript-eslint/no-require-imports` justifié + cross-ref `lib-docs/expo/LESSONS.md:34` "app.config.ts est loadé par Node require — helpers DOIVENT être inlined"). Sentinel `scripts/sentinels/fe-version-sync.mjs` (~138 lignes) wired 3-point : (1) root `package.json` script `sentinel:fe-version-sync` alpha-ordered, (2) `.husky/pre-commit` Gate 8/8 conditional sur staged FE files (budget < 500ms), (3) `.github/workflows/sentinel-mirror.yml` step Mirror Gate P21 inconditionnel (catch `--no-verify` bypass UFR-020). Tests R2a/R2b verrouillent exit 0 happy + exit 1 drift avec diff message. | grep `'1.2.3'` dans `app.config.ts` désormais 0 hits ; `pnpm sentinel:fe-version-sync` exit 0. | shipped |
| ✅ **P0.A6** | **Cost circuit breaker fail-CLOSED wired** — shipped 2026-05-21 (`/team` run `2026-05-21-p0-c2-cost-breaker`, APPROVED 88.5). `canAttempt()` câblé dans `LangChainChatOrchestrator.generate()` (`langchain.orchestrator.ts:333`) AVANT `runSectionTasks` via helper `checkCostBreakerOrThrow`. Quand OPEN → `throw new CircuitOpenError()` → 503 `CIRCUIT_BREAKER_OPEN` via `mapOrchestratorError` (`chat-message.service.ts:62`). `recordFailure()` câblé HALF_OPEN probe failure via `maybeRecordHalfOpenProbeFailure` aggregate-based (`:182-200`). Couvert par `langchain-orchestrator-cost-breaker.spec.ts` + `chat-cost-breaker-503.integration.test.ts`. | grep `costBreaker.canAttempt(` désormais 2 hits (default + walk paths). | shipped |
| ✅ **P0.A7** | **Walk path applies both breakers (parité default)** — shipped 2026-05-21 (même run). `generateWalk` applique `checkLatencyBreakerOrThrow` + `checkCostBreakerOrThrow` AVANT `structured.invoke` (`langchain.orchestrator.ts:556-560`). `recordFailure()` HALF_OPEN câblé via try/catch autour `invokeWalkStructured` (`:478-498`). | Walk path bypass fermé, parité default path R3 verified. | shipped |
| ✅ **P0.A8** | **Honesty docstring fix (a) UFR-013** — shipped 2026-05-21 (même run). `llm-cost-circuit-breaker.ts:1-56` réécrit : (i) 0 hit sur « PHASE 2 PRIMITIVE, NOT WIRED » et « No production caller invokes recordCharge() yet », (ii) caller list factuelle ligne 17 (recordCharge / canAttempt / recordFailure / getState), (iii) cite UFR-013 + RUN_ID `2026-05-21-p0-c2-cost-breaker` comme correction event, (iv) documente image cost override (`VISION_BYTES_EQUIVALENT`). Partie (b) `deleteAccount.useCase.ts:63-64` reste ouverte, à traiter dans le run delete-account (out-of-scope C2). | grep verifier-contract OK. | shipped (a) — (b) backlog |
| ✅ **P0.A9** | **OAuth callback `code` in SENSITIVE_QUERY_KEYS** — subsumed by P0.A3, shipped 2026-05-21. `code` + `state` ajoutés à `SENSITIVE_QUERY_KEYS` canonical (`packages/musaium-shared/src/observability/sentry-scrubber.ts:29-41`). | grep `SENSITIVE_QUERY_KEYS` désormais 11 entries. | shipped |
...
| ✅ **P0.B17** | **`ANTHROPIC_API_KEY` dead placeholder** — shipped 2026-05-21 (`/team` run `2026-05-21-p0-c4-infra`, APPROVED 92.75). Ligne `ANTHROPIC_API_KEY=` retirée de `museum-backend/.env.example:128` (R10). Cross-verifié pas de dep `@anthropic-ai/sdk` (`pnpm-lock.yaml` 0 hits), pas d'import (`grep -rn 'ANTHROPIC' museum-backend/src museum-backend/scripts museum-backend/package.json` = 0 hits). Cleanup hygiène (UFR-016 bury dead code) — pas de credentials à revoke (le placeholder était vide `ANTHROPIC_API_KEY=` sans valeur). Note : `.env` prod actuel (P0.C6 séparé) peut contenir une vraie key à auditer/revoke Tim-side. | grep `ANTHROPIC` dans `.env.example` désormais 0 hits. | shipped |
...
- **C7.1** smoke:api couverture e2e ; C9.2 imageDescription audio-desc autoplay + C9.3a granular consent + C9.3b AI Act Art.50 badge persistent + i18n 8/8 + C9.4 (telemetry side + enforcement gate shipped 2026-05-21 via run C2 cost-breaker, cf P0.A6) + C9.5 stable-prefix + C9.6 Promise.all + C9.7 judge detached + C9.9 output O3 retired + C9.10 voiceMode 80w + C9.11 sandwich dedup + C9.12 TTS Opus + fire-and-forget + cache key v2 + C9.13 reranker scaffold + C9.14 SigLIP-2 + C9.15 retire Google CSE/SearXNG/DDG + C9.17 [META] retired
...
| ID | Conf | Item | Preuve |
|---|---|---|---|
| ✅ **I-SEC1** | ◇◇ | **Redis prod sans `maxmemory`/eviction → fix volatile-ttl + BullMQ caveat** — shipped 2026-05-21 (`/team` run `2026-05-21-p0-c4-infra`, APPROVED 92.75). Prod compose (`deploy/docker-compose.prod.yml:367-378`) append `--maxmemory ${REDIS_MAXMEMORY:-512mb}` + `--maxmemory-policy ${REDIS_MAXMEMORY_POLICY:-volatile-ttl}` (R5). Dev compose `museum-backend/docker-compose.dev.yml` mirror policy (cap libre). Sentinel `scripts/sentinels/compose-parity.mjs:65-75` `CRITICAL_FLAGS` étendu : `--maxmemory-policy` (critical) + `--maxmemory` (warn). Tests R7 verrouillent l'invariant cache TTL (3600s) < counter TTL (90000s) → counters préservés sous pression mémoire (`tests/unit/shared/llm-cost-guard/redis-volatile-ttl-policy.test.ts` + helper `volatile-ttl-sim.ts`). Trade-off architectural locké **ADR-065** : BullMQ no-TTL keys jamais évincées → caveat extreme pressure documenté + alternative split-instance (cache evictable + counters never-evict) reservée V1.x si bake `evicted_keys` révèle insuffisance. **Pas d'isolation compteurs** dans cette run (alternative ADR-065 §Alternatives #2 deferred V1.x conformément au scope C4). | grep `--maxmemory-policy` désormais 2 hits (prod+dev) ; `pnpm sentinel:compose-parity` exit 0. | shipped |
| ✅ **I-SEC2** | ◇◇ | **Cost estimation image-aware** — shipped 2026-05-21 (`/team` run `2026-05-21-p0-c2-cost-breaker`). `VISION_TOKEN_EQUIVALENT = 1000` + `VISION_BYTES_EQUIVALENT = 4000` exportés depuis `llm-cost-pricing.ts:60-61`. `estimatePayloadBytes` (`llm-prompt-builder.ts:469-491`) discrimine via `isImageUrlItem` predicate : items `{type:'image_url', image_url:{url}}` comptés `VISION_BYTES_EQUIVALENT` (4000) au lieu du base64 inflated (×100-1000). Source-agnostic (data-URL base64 ou http URL → même bound — D2 design). `estimateCostCents` < 50 cents pour 1 image + 500 char text (R5 verified `llm-cost-pricing.test.ts:120-127`). Pas de tokenizer dep introduit (R7). | grep `VISION_BYTES_EQUIVALENT` désormais 2 hits (export + caller). | shipped |
| ✅ **I-SEC3** | ◇ | **`POST /chat/art-keywords`** — shipped 2026-05-21 (`/team` run `2026-05-21-p0-c3-auth-crypto`). `requireRole(UserRole.ADMIN, UserRole.MODERATOR)` (super_admin implicite) + `taxonomyWriteLimiter` 10/min keyed `byUserId` désormais montés sur la route (`chat-message.route.ts:184`). visitor → 403 `FORBIDDEN` ; admin/moderator/super_admin → 200 ; 11ème POST en burst <60s → 429. `GET /art-keywords` reste `isAuthenticated` seul (lecture). | grep `requireRole(UserRole.ADMIN, UserRole.MODERATOR)` désormais ≥1 hit en chat-message route. | shipped |
...
| ✅ **I-SEC5** | ◇◇ | **`EXPORT_PSEUDONYM_SALT`** — shipped 2026-05-21 (`/team` run `2026-05-21-p0-c3-auth-crypto`). Fallback littéral retiré des 2 sites (`admin-export.repository.pg.ts` + `exportChatSessions.useCase.ts` via `composition.ts:resolveExportPseudonymSalt`) ; `env.production-validation.ts` ajoute `validateExportPseudonymSalt` (REQUIRED + ≥32 chars en prod, boot fail-fast — mirror `validateCsrfSecret`). Doctrine rotation = privacy-incident-only documentée dans `docs/SECURITY.md#export-salt-rotation`. Initial green a fermé 1/2 site → security-WARN HIGH a déclenché un re-spawn green-corrective fresh-context qui a fermé les 3 sites résiduels (R2 acceptance criterion `git grep "musaium-admin-export-v1" museum-backend/src/` = 0 hits, verifié reviewer 21:55Z). | grep `musaium-admin-export-v1` = 0 hits dans `museum-backend/src/`. | shipped |
...
| ✅ **I-SEC7** | ◇ | **TOTP replay + access-token revocation** — shipped 2026-05-21 (`/team` run `2026-05-21-p0-c3-auth-crypto`). **I-SEC7a (replay)** : migration `AddTotpLastUsedStep` ajoute `last_used_step bigint NULL` sur `totp_secrets` (nullable-then-stamp, zero-downtime) ; `verifyTotpCode` retourne `{step} \| null` ; `challengeMfa`/`verifyMfa` rejettent 401 `INVALID_MFA_CODE` si `acceptedStep <= lastUsedStep` ; `markUsed(uid, at, step)` persiste atomiquement step+`lastUsedAt` via single `repo.update` (TypeORM gotcha `String(step)`, jamais `undefined`). RFC 6238 §5.2 fermé. **I-SEC7b (revocation)** : port `IAccessTokenDenylist` dans `auth/domain/session/` ; adapter Redis `denylist:access:<jti>` SET EX NX, fail-OPEN à l'adapter (warn `access_token_denylist_unavailable` rate-limité 1/min, cf **ADR-064**) ; `verifyAccessTokenWithClaims` sibling additif (D3) expose `jti+expSec` au middleware ; `isAuthenticated`/`isAuthenticatedJwtOnly` consultent denylist post-`jwt.verify` → 401 `TOKEN_REVOKED` ; `authSession.logout(refreshToken, {accessJti, accessExpSec})` denylist + revoke refresh. Adapters Noop + in-memory pour dev/tests (D6). | grep `IAccessTokenDenylist` désormais ≥1 hit en port ; migration `1779391176767-AddTotpLastUsedStep.ts` présente. | shipped |
...
| ✅ **I-SEC10** | · | **KE scraper OOM/DoS — 2-layer Content-Length cap** — shipped 2026-05-21 (`/team` run `2026-05-21-p0-c4-infra`, APPROVED 92.75). `html-scraper.ts:187-242` refactor : helper `readBodyWithCap()` extrait (~70 lignes single-purpose). **Layer 1** Content-Length pre-guard (`:187-198`) — `Number.parseInt` + `Number.isFinite` handle NaN/Infinity/negative safely ; si header présent + valide + > `maxContentBytes` → `logger.warn('scraper_payload_too_large')` + return null SANS consommer body (R8). **Layer 2** streamed cap (`:201-242`) — body lu via `response.body.getReader()` (Web ReadableStream WHATWG, natif Node 22), `TextDecoder` incrémental (`{fatal:false}` survit UTF-8 mid-byte truncate), cumulative bytes check ; à l'overflow → `reader.cancel()` + `logger.warn('scraper_payload_streamed_overflow')` + return null (R9). Defense-in-depth contre Content-Length falsifié (D8). Mémoire bornée O(`maxContentBytes`) confirmé via test R9a (lecture max `ceil(cap/chunk)+1` chunks). SSRF guards préservés `:88-126` + `:240-247`. | grep `await response.text()` dans `html-scraper.ts` désormais 0 hits ; tests R8a/R8b/R9a/R9b PASS. | shipped |
...
- ~~**Cost breaker docstring** "PHASE 2, NOT WIRED, no caller invokes recordCharge()" — `recordCharge()` EST wiré ; c'est l'enforcement (`canAttempt()`) qui manque (P0.A6). Docstring à corriger.~~ **Résolu 2026-05-21** dans le run C2 (P0.A8 (a)) — `llm-cost-circuit-breaker.ts:1-56` réécrit, UFR-013 cité.
...
- [ ] **W6.1 Llama-Prompt-Guard-2-86M wire vs delete** — décision P0.D3 lock. Si wire : sidecar FastAPI + shadow run 7j (observeOnly) puis swap.
...
- [x] **Sentry `event.tags` walked by `scrubEvent`** — shipped 2026-05-21 (`/team` run `2026-05-21-p0-c1-pii-egress`, R2). `scrubEvent` traverse désormais `event.tags` via `scrubRecord` (clés sensibles → `[redacted]`) + `scrubUrl` opportuniste sur valeurs URL-like (`packages/musaium-shared/src/observability/sentry-scrubber.ts:220-232`). Bumped en P0.A3.
