...
> 1. 🔴 **P0-FA1 — Chat texte-seul → bulle assistant VIDE** (image/audio OK). `sendMessageStreaming.ts:117` garde `(!streamingIdRef.current || imageUri)` falsy en sync car `sendMessageSmart` (`send.ts:169-172`) n'appelle plus `onDone` → placeholder `text:''` jamais rempli. 3 agents + trace byte-level ; tests verts car ils mockent le path mort (`useChatSession.test.ts:773`). **Confirmer device** puis fix garde + test texte-seul + Maestro.
...
> 4. 🟠 **P0-FA4 — KR2 NPS non livré** : `aggregateNps()`/`findByMuseum()` (`review.repository.pg.ts:88`) = 0 caller ; `StarRating.tsx:31` plafonné à 5 → impossible de produire 0-10. Câbler ou descoper KR2 honnêtement.
...
> 6. 🟠 **P0-FA6 — `museum_manager` cassé** : `getStats.useCase.ts:24` ignore museumId → fuite agrégat global ; 7 liens nav → 403 ; branding W2.2 write-to-void (claim faux `branding/page.tsx:24`). Aligner FE/BE ou retirer le rôle V1.
...
>
> **⚠️ Honnêteté (UFR-013)** : `audit-state/.../LOT-P0-STABILITY-CLOSURE.md` (committé `0349095c5` aujourd'hui) prétend LOT 4 (I-OPS2/3/4/6/7, I-FIX3, I-SEC8) fixé sur `p0/stability` — **AUCUN code sur `dev`** (SHAs non-ancêtres, fichiers introuvables). LOT 4 reste NON DÉMARRÉ. À corriger/retirer. · Doc-anchors `c4b-sparql-counts.md`/`c2-license-uris.md` référencés 8× mais inexistants (UFR-024 cassé). · I-SEC8 reclassé CRITIQUE→**LOW** (catalogue public scrapé, ADR-061 cohérent ; consensus B5+C6 vs A2). · I-SEC4/I-SEC6 textes "live" à réécrire (bugs fixés/jamais réels).
...
| ID | Item | Preuve | Effort |
|---|---|---|---|
| ✅ **P0.C1** | *(DONE-DEV — LOT 3 feature-gates mergé `811fd501c` #295)* **SigLIP model provisioning** — `Dockerfile.prod:38` runs `fetch-models.sh` mais script avale 404 silencieusement quand `SIGLIP_ONNX_SHA256` unset. GCS bucket `musaium-models-public` TODO `fetch-models.sh:36-40`. Prod default `EMBEDDINGS_PROVIDER=siglip-onnx` → cold start /chat/compare = 503. Soit bake dans Docker, soit provision GCS + pin SHA256. | Agents 01 A+B Theme 2 | 1-2h |
