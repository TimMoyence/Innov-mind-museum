...
|---|---|---|---|
| ❌ **P0.A1** | **Email clair-texte 6 sites (pas 5 comme claim antérieur)** — créer `@shared/pii/extractEmailDomain.ts` + patcher : `auth/useCase/forgotPassword.useCase.ts:33,53,60` + `login-handler.helpers.ts:65,75` + `auth-password.route.ts:60` | Agent 01 confirmé. Logger sans scrub layer ; audit metadata `LooseRecordSchema`. | 60-90 min |
| ❌ **P0.A2** | **DOB bypass exploitable curl direct** — drop `.optional()` `auth.schemas.ts:18` + drop fallback `if(!dateOfBirth) return` `register.useCase.ts:106` + fix test 421-430 + bump OpenAPI `required[]:2298` | Exploit confirmé `curl -X POST /api/auth/register -d '{"email":"minor@test.fr","password":"V1"}'` → 201 + null DOB | 2-3h |
