...
>
> **Anchors/sentinels cassés (UFR-024)** : `c4b-sparql-counts.md`/`c2-license-uris.md` (8 réfs code mortes) · `doc-anchor-check.mjs` cité dans CLAUDE.md **n'existe pas** · `LOT-P0-STABILITY-CLOSURE.md` = claim creux (commits orphelins non-ancêtres, LOT 4 jamais mergé) · `_rtl-style-audit.ts` ne couvre pas les border-radius (RTL bug `SwipeableConversationCard` non-flaggé) · 2 Maestro stale actifs sur shards CI (`audio-recording-flow.yaml`, `onboarding-flow.yaml`).
