...
|---|---|---|---|---|
| L1 | **Keyword pre-filter** (`art-topic-guardrail.ts`) — `normalize()` defangs obfuscation before the scan (H10): zero-width strip, curated Cyrillic+Greek→Latin homoglyph fold, base64-candidate decode | input, before any other layer | every prompt | deterministic — zero upstream dependency |
...
**L1 obfuscation defang (H10).** Before the keyword scan, `normalize()` neutralises three evasion classes so a keyword spliced/disguised by an attacker still matches: (a) **zero-width strip** removes invisible separators (U+200B-200D, U+2060, U+FEFF) an attacker inserts inside a keyword (`ign​ore previous`); (b) **homoglyph fold** maps a *curated* Cyrillic (U+04xx) + Greek (U+03xx) look-alike set to Latin (Cyrillic `о`→`o`, Greek omicron `ο`→`o`, …) — deliberately NOT a full transliteration, and CJK/Arabic ranges are absent, so a genuine Russian/Greek art question folds to Latin gibberish (never into an injection pattern) and the multilingual allow-cases stay green; (c) **base64-candidate decode** runs on the RAW case-preserved value, matches ≥16-char base64-alphabet runs, decodes each, and appends only decodes that look like printable prose (≥85% printable ASCII AND contain a letter) — hex hashes and base64-shaped artwork/inventory tokens decode to binary garbage and are dropped, so legit art chat is not over-blocked. This raises L1 recall on obfuscated keyword attacks without weakening the false-positive profile; it does not turn L1 into a semantic detector (novel non-keyword attacks still rely on L4/L5).
