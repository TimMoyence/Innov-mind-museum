...
- [x] **Guardrails V2 — flag retirée entièrement** (au lieu du calibrage proposé en V1). `GUARDRAILS_V2_CANDIDATE` supprimé du code (env-resolvers + env.types + env.ts + chat-module). llm-guard self-activé via présence URL ; judge layer self-activé via `LLM_GUARDRAIL_BUDGET_CENTS_PER_DAY > 0` (défaut 0). ADR-015 amendée 2026-05-14, ADR-047 reste valide. Cohérent `feedback_no_feature_flags_prelaunch`. Les `.env*` et docker-compose gardent la ligne `GUARDRAILS_V2_CANDIDATE=` historique en dead config (user/linter trade-off — tolère le bruit pour éviter une seconde wave d'edits).
