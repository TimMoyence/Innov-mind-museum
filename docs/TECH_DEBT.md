...
## TD-FE-CHAT-BURY-SSE — Clôturé

- [x] **Statut** : clôturé le 2026-09-15.
- **Correction** : la voie de transport est maintenant synchrone de bout en bout. Les callbacks SSE morts, l'état de streaming associé et les tests qui simulaient ce transport ont été supprimés. La stratégie active et ses tests couvrent désormais la réponse métier réelle.
- **Validation** : `npm test`, `npm run lint`, les tests affectés et les sentinelles OpenAPI passent ; l'ancien endpoint reste uniquement marqué `deprecated` dans le contrat pour préserver la compatibilité documentaire avec les clients historiques.
