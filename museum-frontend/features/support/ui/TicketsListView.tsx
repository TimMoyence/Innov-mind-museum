...
import { STATUS_OPTIONS } from '@/features/support/application/useTicketsListScreen';
import {
  BADGE_TEXT_COLOR,
  statusColor,
  priorityColor,
...
/**
 * Props for `TicketsListView` — exact return shape of `useTicketsListScreen`.
 * Declared locally so `react/prop-types` can statically resolve each prop
 * (the rule does not follow cross-file type aliases).
 */
export interface TicketsListViewProps {
  ...
  loadTickets: (requestedPage: number, isRefresh?: boolean) => Promise<void>;
  loadMore: () => Promise<void>;
  handleStatusFilter: (newStatus: TicketStatus | 'all') => void;
  statusLabel: (s: TicketStatus | 'all') => string;
  dismissError: () => void;
}

/** Pure presenter for the tickets list — receives state from `useTicketsListScreen`. */
export const TicketsListView = ({
  ...
  return (
    ...
    >
      ...
      {error ? (
        <ErrorState variant="inline" title={error} onDismiss={dismissError} testID="error-notice" />
      ) : null}
