...
const createRule = utils_1.ESLintUtils.RuleCreator((name) => `https://github.com/innovmind/musaium/blob/main/tools/eslint-plugin-musaium-test-discipline/README.md#${name}`);
            ...
            Program(programNode) {
                ...
                for (const comment of comments) {
                    if (comment.type !== utils_1.AST_TOKEN_TYPES.Line &&
                        comment.type !== utils_1.AST_TOKEN_TYPES.Block) {
                        continue;
                    }
                    ...
                    const targetsOurPlugin = disabledRules.some((r) => TARGET_RULES.includes(r) || r.startsWith(TARGET_PLUGIN + '/'));
                    if (!targetsOurPlugin)
                        continue;
                    // Require Justification: with ≥20 chars body (stopped before Approved-by:),
                    // and Approved-by: with ≥1 char
                    const justificationMatch = justification.match(/Justification:\s*(.*?)(?:\s+Approved-by:|$)/);
